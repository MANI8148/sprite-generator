...
class TestGenerate:
    ...
    def test_generate_with_various_asset_types(self, client):
        for asset_type in ["character", "building", "vehicle", "enemy", "prop"]:
            ...
            result = poll_job(client, data["job_id"])
            assert result["status"] == "done"
            assert result["quality_tier"] in ("clean", "acceptable", "noisy", "blurry", "broken_outline", "empty", "extreme_aspect")
...
class TestBatchGenerate:
    def test_batch_generate_two_items(self, client):
        ...
        assert result["results"][0]["quality_tier"] in ("clean", "acceptable", "noisy", "blurry", "broken_outline", "empty", "extreme_aspect")
        assert result["results"][1]["quality_tier"] in ("clean", "acceptable", "noisy", "blurry", "broken_outline", "empty", "extreme_aspect")
    ...
    def test_batch_generate_multiple_asset_types(self, client):
        ...
        for r in result["results"]:
            assert r["quality_tier"] in ("clean", "acceptable", "noisy", "blurry", "broken_outline", "empty", "extreme_aspect")
